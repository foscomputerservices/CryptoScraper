...
public extension BscScan {
    ...
    func getBalance(forToken contract: CryptoContract, forAccount account: CryptoContract) async throws -> CryptoAmount {
        // Cannot retrieve BNB contract, but retrieve BNB balance
        if contract.isChainToken {
            ...
            return try response.cryptoBalance(forToken: contract, bnbContract: account.chain.mainContract)
        }
    }
...
private struct TokenBalanceResponse: Decodable {
    ...
    func cryptoBalance(forToken: CryptoContract, bnbContract: CryptoContract) throws -> CryptoAmount {
        guard success else {
    ...
    // https://docs.bscscan.com/api-endpoints/tokens#get-erc-20-token-account-balance-by-contractaddress
...
private struct TokenInfoResponse: Decodable {
    ...
    // https://docs.bscscan.com/api-endpoints/tokens#get-token-info-by-contractaddress
...
private struct TokenInfo: Decodable {
    ...
    private struct MappedInfo: CryptoInfo {
        ...
        init(tokenInfo: TokenInfo) {
            let contract = BNBContract(address: tokenInfo.contractAddress)
